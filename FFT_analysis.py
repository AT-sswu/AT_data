...
# FFT 분석 함수
def fft_analysis(data, sample_rate=1000, duration=2, plot=True): #sample_rate 조정 필요
    # 데이터 평균 제거
    data = data - data.mean()
    # 샘플 수 계산
...
# CSV 파일 읽기 + FFT 분석
def analyze_csv_fft(file_path, axis='Accel_X', sample_rate=1000, duration=2):
...
# 분석 함수 실행
analyze_csv_fft(r"C:\Users\USER\PycharmProjects\AT_data\mpu6050_static_data.csv", axis='Accel_X')
